...
class MsuMetadata:
    ...
    def __extractData(self):
        ...
        self.create_time = datetime.datetime.fromisoformat(self.root.get('CreatedDate'))
...
def extractMsuWindowsLegacy(main_cab: str, file_name: re.Pattern[str], output_dir: str, silent: bool = False):
    ...
    last_kb = re.search(r'(-|_)(?P<kb>(KB\d+))(-|_)', main_cab, re.I).group('kb')
    with TmpDir() as tmp_dir:
        for sub_cab in expandExtractFiles(main_cab, tmp_dir, False, ['Cab_*.cab']):
            kb = re.search(r'Cab_\d+_for_(?P<kb>(KB\d+))(_PSFX)?\.cab', sub_cab, re.I).group('kb')
            if last_kb and last_kb.upper() != kb.upper():
                raise SymbolManagerException(f'KB sanity check failed! {last_kb} != {kb}')
            is_diff_style = is_diff_style or (re.search(r'_PSFX\.cab', sub_cab, re.I) is not None)
            last_kb = kb.upper()
            extracted_files += list(expandExtractFiles(sub_cab, output_dir, False, getInterestingFiles()))
    if len(extracted_files) == 0:
        extracted_files += list(expandExtractFiles(main_cab, output_dir, False, getInterestingFiles()))
...
def extractMsuWindowsServer(msu_file_path: str, file_name: re.Pattern[str], output_dir: str, silent: bool = False):
    with TmpDir() as tmp_dir, TmpDir() as cab_tmp_dir:
        main_cabs = z7ExtractFiles(msu_file_path, tmp_dir, file_filters=[f'Windows*.*-KB*-x*.cab'])
        if len(main_cabs) != 1:
            raise SymbolManagerException(f'Main CAB of Windows Server patch was not extracted properly!')
        main_cab = os.path.join(tmp_dir, main_cabs[0])
        sub_cabs = z7ExtractFiles(main_cab, cab_tmp_dir, file_filters=[f'Windows*.*-KB*-x*.cab'])
        if len(sub_cabs) != 1:
        ...
        kb_sanity1 = re.search(r'Windows\d+.\d+-(?P<kb>(KB\d+))-x\d+\.cab', sub_cabs[0], re.I).group('kb')
        sub_cab = os.path.join(cab_tmp_dir, sub_cabs[0])
        psf_cab2s = z7ExtractFiles(sub_cab, cab_tmp_dir, file_filters=[f'Cab_2_for_KB*_PSFX.cab'])
        if len(psf_cab2s) != 1:
            # In older versions there is only 1 layer of CABs
            printInfo(f'PSF CAB2 of Windows Server patch was not extracted properly!')
            # Pretend that we found it
            psf_cab2 = sub_cab
        else:
            kb_sanity2 = re.search(r'Cab_2_for_(?P<kb>(KB\d+))_PSFX\.cab', psf_cab2s[0], re.I).group('kb')
            if kb_sanity1.lower() != kb_sanity2.lower():
                raise SymbolManagerException(f'Could not verify KB number! {kb_sanity1} != {kb_sanity2}')
            psf_cab2 = os.path.join(cab_tmp_dir, psf_cab2s[0])
...
def extractMsu(msu_file_path: str, file_name: re.Pattern[str], output_dir: str, silent: bool = False):
    with TmpDir() as tmp_dir, TmpDir() as cab_tmp_dir:
        ...
        metadata_files = list(expandExtractFiles(msu_file_path, tmp_dir, file_filters=[f'*Metadata.cab']))
        try:
            ...
            printError(f'Failed to extract MSU {msu_file_path} : {metadata_files}')
            raise SymbolManagerException(f'Metadata file was not properly extracted from the MSU!')
        
        metadata_file = os.path.join(tmp_dir, metadata_files[0])
        
        lcu_metadata_files = z7ExtractFiles(metadata_file, tmp_dir, file_filters=[f'LCU*.xml.cab'])
        if len(lcu_metadata_files) != 1:
            raise SymbolManagerException(f'LCU metadata file was not properly extracted from the MSU\'s metadata CAB!')
        lcu_metadata_file = os.path.join(tmp_dir, lcu_metadata_files[0])

        kb_reg = re.search(r'_(?P<kb>(KB\d+))\.xml\.cab$', lcu_metadata_file, re.I)
        if not kb_reg:
            raise SymbolManagerException(f'Failed to find KB number from {lcu_metadata_file}')
        kb = kb_reg.group('kb')

        metadata_manifests = z7ExtractFiles(lcu_metadata_file, tmp_dir, file_filters=[f'*.xml'])
        if len(metadata_manifests) != 1:
            raise SymbolManagerException(f'Metadata manifest file was not properly extracted from the MSU\' metadata CAB\'s LCU CAB!')
        metadata_manifest = os.path.join(tmp_dir, metadata_manifests[0])

        msu_metadata = MsuMetadata(metadata_manifest)
        printInfo(f'Parsing MSU for updating base version {msu_metadata.os_base_version} to patched version {msu_metadata.os_target_version} : ({msu_file_path})')

        payload_files = z7ExtractFiles(msu_file_path, tmp_dir, file_filters=[payload.payload_path for payload in msu_metadata.payloads])
        if len(payload_files) != 2:
            raise SymbolManagerException(f'Unexpected amount of payload items found in LCU!')
        ...
        express_file_paths = z7ExtractFiles(cab_file_path, cab_tmp_dir, file_filters=['express.psf.cix.xml'])
        if len(express_file_paths) != 1:
            raise SymbolManagerException(f'Express PSF XML file was not properly extracted from the MSU\'s CAB file!')
